...
class TransactionControllerTest {
  ...
  void getTransactions_returnsListOfTransactions() throws Exception {
    ...
    verify(transactionService, times(1)).getTransactions(anyString());
  }
  ...
  @Test
  void batchImport_duplicateTransactions_returnsNoTransactionsCreatedError() throws Exception {
    ...
    // When/Then: POST returns 422 because no source-backed import was recorded
    mockMvc
        .perform(
            post("/v1/transactions/batch")
                .with(
  ...
  @Test
  void batchImport_previewImportToken_mapsVerifiedSourceToService() throws Exception {
    when(previewImportTokenService.verifyToken("preview-token", "test-user"))
        .thenReturn(
  ...
  @Test
  void batchImport_missingPreviewImportToken_returns400BeforeService() throws Exception {
    var requestBody =
        """
        {
          "transactions": [
  ...
  void batchImport_invalidPreviewImportToken_returns422BeforeService() throws Exception {
    when(previewImportTokenService.verifyToken("bad-token", "test-user"))
        .thenThrow(
            new BusinessException(
                "Preview import token format is invalid.",
                BudgetAnalyzerError.PREVIEW_IMPORT_TOKEN_INVALID.name()));
  ...
  void searchTransactions_returnsPagedResponseContract() throws Exception {
    var transaction =
        createTransactionWithOwner(1L, "usr_owner1", "Coffee Shop", BigDecimal.valueOf(4.50));
  ...
  void searchTransactions_ownerIdPresentInEachItem() throws Exception {
    var transactions =
        List.of(
            createTransactionWithOwner(1L, "usr_owner1", "Groceries", BigDecimal.valueOf(50.00)),
            createTransactionWithOwner(2L, "usr_owner2", "Gas", BigDecimal.valueOf(35.00)));
  ...
  @Test
  void countTransactionsAcrossUsers_returnsCount() throws Exception {
  ...
  @Test
  void countTransactionsAcrossUsers_filterParams_bindCorrectly() throws Exception {
    when(transactionService.countNotDeleted(any())).thenReturn(7L);
  ...
  private Transaction createTransactionWithOwner(
